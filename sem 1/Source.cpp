...
void main() {
	...
	//1 byte - 8 biti
	int x = 33; //var. intreaga - 4 bytes
	float y = 44.33; //var. reala - 4 bytes
	double z = 99.33; //var.reala - 8 bytes
	char w = 'p'; //var. caracter - 1 byte

	char sir[6] = { 'R','a','r','e','s','\0' };//sir de caractere

	const int aaa = 89;//variabila constanta intreaga. nu poate fi modificata;

	//%d = format de afisare pentru int
	//%lf = format de afisare pentru double
	//%f = format de afisare pentru float
	//%c = format de afisare pentru caractere
	//%s = format de afisare pentru siruri de caractere
	...
	//pointeri
	printf("\n\n---pointeri---\n\n");
	//sa presupunem ca variabila var1 este salvata la adresa #22
	int* pointer1 = &var1; //variabila pointer este si ea creata la o adresa de memorie #3 si are ca valoarea #22
	printf("pointer1=%p\n", pointer1);
	printf("*pointer1=%d\n", *pointer1);//se numeste indirectare

	*pointer1 = *pointer1 + 20;//se numeste indirectare - adica se acceseaza zona de memorie #22 si se incrementeaza cu 10
	...
	//int (*pointerLaFunctie2)(int, int);
	//pointerLaFunctie2 = suma;
